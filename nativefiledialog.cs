...
public static class nativefiledialog
{
	...
	public unsafe static nfdresult_t NFD_OpenDialog(string filterList, string defaultPath, out string outPath)
	{
		...
		IntPtr outPath2;
		nfdresult_t handle = INTERNAL_NFD_OpenDialog(intPtr, ptr, out outPath2);
	...
	{
		byte* intPtr = Utf8EncodeNullable(filterList);
		byte* num = Utf8EncodeNullable(defaultPath);
		IntPtr ptr;
		nfdresult_t chars = INTERNAL_NFD_SaveDialog(intPtr, num, out ptr);
		Marshal.FreeHGlobal((IntPtr)intPtr);
		Marshal.FreeHGlobal((IntPtr)num);
		outPath = UTF8_ToManaged(ptr, freePtr: true);
		return chars;
	}

...
namespace Terraria
{
	public class Collision
	{
		...
		public static bool DrownCollision(Vector2 Position, int Width, int Height, float gravDir = -1f, bool includeSlopes = false)
		{
			...
			for (int i = num3; i < value2; i++)
			{
				for (int j = value3; j < value4; j++)
				{
					...
					if (tile != null && tile.liquid > 0 && !tile.lava() && (j != num4 || !tile.active() || !Main.tileSolid[tile.type] || Main.tileSolidTop[tile.type] || (includeSlopes && tile.blockType() != 0)))
					{
						vector2.X = i * 16;
						vector2.Y = j * 16;
						int num5 = 16;
		...
		public static Vector2 HurtTiles(Vector2 Position, Vector2 Velocity, int Width, int Height, bool fireImmune = false)
		{
			...
			for (int i = num; i < num2; i++)
			{
				for (int j = num3; j < num4; j++)
				{
					...
					if (vector.X + (float)Width >= vector2.X && vector.X <= vector2.X + 16f && vector.Y + (float)Height >= vector2.Y && vector.Y <= vector2.Y + (float)num6 + 0.5f)
					{
						int num9 = 1;
						if (vector.X + (float)(Width / 2) < vector2.X + 8f)
						{
		...
		public static void StepConveyorBelt(Entity entity, float gravDir)
		{
			...
			{
				if (!WorldGen.InWorld(item.X, item.Y) || (player != null && player.onTrack && item.Y < num3))
				{
					continue;
				}

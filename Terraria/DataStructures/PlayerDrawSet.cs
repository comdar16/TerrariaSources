...
namespace Terraria.DataStructures
{
	public struct PlayerDrawSet
	{
		...
		public void BoringSetup(Player player, List<DrawData> drawData, List<int> dust, List<int> gore, Vector2 drawPosition, float shadowOpacity, float rotation, Vector2 rotationOrigin)
		{
			...
			if (drawPlayer.body > 0 && drawPlayer.body < 246)
			{
				Main.instance.LoadArmorBody(drawPlayer.body);
			}
			if (drawPlayer.legs > 0 && drawPlayer.legs < 234)
			{
				Main.instance.LoadArmorLegs(drawPlayer.legs);
			}
			if (drawPlayer.handon > 0 && drawPlayer.handon < 23)
			{
				Main.instance.LoadAccHandsOn(drawPlayer.handon);
			}
			if (drawPlayer.handoff > 0 && drawPlayer.handoff < 15)
			{
				Main.instance.LoadAccHandsOff(drawPlayer.handoff);
			}
			if (drawPlayer.back > 0 && drawPlayer.back < 35)
			{
				Main.instance.LoadAccBack(drawPlayer.back);
			}
			if (drawPlayer.front > 0 && drawPlayer.front < 12)
			{
			...
			if (drawPlayer.balloon > 0 && drawPlayer.balloon < 19)
			{
				Main.instance.LoadAccBalloon(drawPlayer.balloon);
			}
			if (drawPlayer.backpack > 0 && drawPlayer.backpack < 35)
			{
				Main.instance.LoadAccBack(drawPlayer.backpack);
			}
			if (drawPlayer.tail > 0 && drawPlayer.tail < 35)
			{
				Main.instance.LoadAccBack(drawPlayer.tail);
			}
			if (drawPlayer.faceHead > 0 && drawPlayer.faceHead < 20)
			{
				Main.instance.LoadAccFace(drawPlayer.faceHead);
			}
			if (drawPlayer.faceFlower > 0 && drawPlayer.faceFlower < 20)
			{
				Main.instance.LoadAccFace(drawPlayer.faceFlower);
			}
			if (drawPlayer.balloonFront > 0 && drawPlayer.balloonFront < 19)
			{
			...
			if (drawPlayer.slowOgreSpit && Main.instance.IsActive && !Main.gamePaused)
			{
				...
				if (Main.rand.Next(5) == 0 && shadow == 0f)
				{
					...
					Dust dust19 = Main.dust[Dust.NewDust(Position, drawPlayer.width, drawPlayer.height, type2, 0f, 0f, 100)];
			...
			if (Main.instance.IsActive && !Main.gamePaused && shadow == 0f)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					{
						Dust dust20 = Dust.NewDustDirect(Position, drawPlayer.width, drawPlayer.height, type3, 0f, 0f, 100, default(Color), 1.5f);
			...
			if (drawPlayer.opacityForCreditsRoll != 1f)
			{
				shadow = 1f - drawPlayer.opacityForCreditsRoll;
				float opacityForCreditsRoll = drawPlayer.opacityForCreditsRoll;
				opacityForCreditsRoll *= opacityForCreditsRoll;
				colorEyeWhites = Color.White * opacityForCreditsRoll;
				colorEyes = drawPlayer.eyeColor * opacityForCreditsRoll;
				colorHair = GameShaders.Hair.GetColor(drawPlayer.hairDye, drawPlayer, Color.White) * opacityForCreditsRoll;
				colorHead = drawPlayer.skinColor * opacityForCreditsRoll;
				colorBodySkin = drawPlayer.skinColor * opacityForCreditsRoll;
				colorShirt = drawPlayer.shirtColor * opacityForCreditsRoll;
				colorUnderShirt = drawPlayer.underShirtColor * opacityForCreditsRoll;
				colorPants = drawPlayer.pantsColor * opacityForCreditsRoll;
				colorShoes = drawPlayer.shoeColor * opacityForCreditsRoll;
				colorLegs = drawPlayer.skinColor * opacityForCreditsRoll;
				colorArmorHead = drawPlayer.GetImmuneAlpha(Color.White, shadow);
				colorArmorBody = drawPlayer.GetImmuneAlpha(Color.White, shadow);
				colorArmorLegs = drawPlayer.GetImmuneAlpha(Color.White, shadow);
			}
			...
			{
				drawFrontAccInNeckAccLayer = drawPlayer.front > 0 && drawPlayer.front < 12 && ArmorIDs.Front.Sets.DrawsInNeckLayer[drawPlayer.front];
			}

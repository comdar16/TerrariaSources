	...
	public class Liquid
	{
		...
		private static void SettleWaterAt(int originX, int originY)
		{
			...
			while (true)
			{
				...
				if (flag3 && WorldGen.gen && !flag && num2 > WorldGen.waterLine)
				{
					b = 1;
				}
				int num4 = -1;
			...
			if ((tile.liquid > 0 && !tile.lava()) || (tile2.liquid > 0 && !tile2.lava()) || (tile3.liquid > 0 && !tile3.lava()))
			{
				...
				else
				{
					...
					if (!WorldGen.gen)
					{
						if (num2 == 56)
						{
							SoundEngine.PlaySound(SoundID.LiquidsWaterLava, new Vector2(x * 16 + 8, y * 16 + 8));
						}
						else
						{
							SoundEngine.PlaySound(SoundID.LiquidsHoneyLava, new Vector2(x * 16 + 8, y * 16 + 8));
						}
					}
		...
		public static void HoneyCheck(int x, int y)
		{
			...
			if ((tile.liquid > 0 && tile.liquidType() == 0) || (tile2.liquid > 0 && tile2.liquidType() == 0) || (tile3.liquid > 0 && tile3.liquidType() == 0))
			{
				...
				if (!tile5.active())
				{
					...
					{
						SoundEngine.PlaySound(SoundID.LiquidsHoneyWater, new Vector2(x * 16 + 8, y * 16 + 8));
					}
					WorldGen.SquareTileFrame(x, y);
					if (Main.netMode == 2)
					{

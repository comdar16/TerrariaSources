	...
	public class ItemSlot
	{
		...
		public static void Draw(SpriteBatch spriteBatch, Item[] inv, int context, int slot, Vector2 position, Color lightColor = default(Color))
		{
			...
			if ((context == 0 || context == 2) && inventoryGlowTime[slot] > 0 && !inv[slot].favorited && !inv[slot].IsAir)
			{
				...
				Color value2 = new Color(63, 65, 151, 255) * num8;
				Color value3 = Main.hslToRgb(inventoryGlowHue[slot], 1f, 0.5f) * num8;
				float num9 = (float)inventoryGlowTime[slot] / 300f;
				num9 *= num9;
				color2 = Color.Lerp(value2, value3, num9 / 2f);
			...
			if ((context == 4 || context == 3) && inventoryGlowTimeChest[slot] > 0 && !inv[slot].favorited && !inv[slot].IsAir)
			{
				...
				Color value4 = new Color(130, 62, 102, 255) * num10;
				if (context == 3)
				{
					value4 = new Color(104, 52, 52, 255) * num10;
				}
				Color value5 = Main.hslToRgb(inventoryGlowHueChest[slot], 1f, 0.5f) * num10;
				float num11 = (float)inventoryGlowTimeChest[slot] / 300f;
				num11 *= num11;
				color2 = Color.Lerp(value4, value5, num11 / 2f);
				value = TextureAssets.InventoryBack13.Value;
			}
			if (flag)
			{
			...
			if ((item.type <= 0 || item.stack <= 0) && num12 != -1)
			{
				...
				Rectangle rectangle = value6.Frame(3, 6, num12 % 3, num12 / 3);
				rectangle.Width -= 2;
				rectangle.Height -= 2;
			...
			if (item.type > 0 && item.stack > 0)
			{
				...
				num13 *= inventoryScale;
				Vector2 position2 = position + vector / 2f - rectangle2.Size() * num13 / 2f;
				Vector2 origin = rectangle2.Size() * (scale / 2f - 0.5f);
				spriteBatch.Draw(value7, position2, rectangle2, item.GetAlpha(currentColor), 0f, origin, num13 * scale, SpriteEffects.None, 0f);
				if (item.color != Color.Transparent)
				{
					...
					spriteBatch.Draw(value7, position2, rectangle2, item.GetColor(newColor), 0f, origin, num13 * scale, SpriteEffects.None, 0f);
				}
				...
				if (num14 != -1)
				{
					ChatManager.DrawColorCodedStringWithShadow(spriteBatch, FontAssets.ItemStack.Value, num14.ToString(), position + new Vector2(8f, 30f) * inventoryScale, color, 0f, Vector2.Zero, new Vector2(inventoryScale * 0.8f), -1f, inventoryScale);
				}
				if (context == 13)
				{
				...
				if (context == 13 && item.potion)
				{
					...
					spriteBatch.Draw(TextureAssets.Cd.Value, position3, null, color3, 0f, default(Vector2), num13, SpriteEffects.None, 0f);
				}
				if ((context == 10 || context == 18) && item.expertOnly && !Main.expertMode)
				{
					...
					spriteBatch.Draw(TextureAssets.Cd.Value, position4, null, white, 0f, default(Vector2), num13, SpriteEffects.None, 0f);
				}
			}
			else if (context == 6)
			{
			...
			if (context == 0 && slot < 10)
			{
				...
				ChatManager.DrawColorCodedStringWithShadow(spriteBatch, FontAssets.ItemStack.Value, text2, position + new Vector2(6f, 4 + num16) * inventoryScale, baseColor, 0f, Vector2.Zero, new Vector2(inventoryScale), -1f, inventoryScale);
			}
			if (num != -1)
			{

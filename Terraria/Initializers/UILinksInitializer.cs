	...
	public class UILinksInitializer
	{
		...
		public static void Load()
		{
			...
			cp19.UpdateEvent += delegate
			{
				...
				for (int num63 = 40; num63 <= 49; num63++)
				{
					if (inReforgeMenu)
					{
						cp19.LinkMap[num63].Down = ((num63 < 45) ? 303 : 304);
					}
					else if (flag7)
					{
						cp19.LinkMap[num63].Down = 400 + num63 - 40;
					}
					else if (flag8)
					{
						cp19.LinkMap[num63].Down = 2700 + num63 - 40;
					}
					else if (num63 == 40)
					{
						cp19.LinkMap[num63].Down = 311;
					}
					else
					{
						cp19.LinkMap[num63].Down = -2;
					}
				}
				if (flag10)
				{
					for (int num64 = 40; num64 <= 47; num64++)
					{
						cp19.LinkMap[num64].Down = 5100 + num64 - 40;
					}
				}
				if (flag9)
				{
					for (int num65 = 44; num65 <= 45; num65++)
					{
						cp19.LinkMap[num65].Down = 5000 + num65 - 44;
					}
				}
				if (flag7)
				{
			...
			{
				int num62 = UILinkPointNavigator.CurrentPoint - 100;
				return ItemSlot.GetGamepadInstructions(Main.player[Main.myPlayer].armor, (num62 < 10) ? 8 : 9, num62);
			};
			...
			cp16.UpdateEvent += delegate
			{
				...
				for (int num59 = 0; num59 < amountOfExtraAccessorySlotsToShow; num59++)
				{
					cp16.LinkMap[num58 + num59].Down = num58 + num59 + 1;
					cp16.LinkMap[num58 - 100 + 120 + num59].Down = num58 - 100 + 120 + num59 + 1;
					cp16.LinkMap[num58 + 10 + num59].Down = num58 + 10 + num59 + 1;
				}
				cp16.LinkMap[num58 + amountOfExtraAccessorySlotsToShow].Down = 308;
				cp16.LinkMap[num58 + 10 + amountOfExtraAccessorySlotsToShow].Down = 308;
				cp16.LinkMap[num58 - 100 + 120 + amountOfExtraAccessorySlotsToShow].Down = 308;
				bool shouldPVPDraw = Main.ShouldPVPDraw;
				for (int num60 = 120; num60 <= 129; num60++)
				{
					UILinkPoint uILinkPoint17 = cp16.LinkMap[num60];
					int num61 = num60 - 120;
				...
				cp16.LinkMap[num58 - 100 + 120 + amountOfExtraAccessorySlotsToShow].Left = 1557;
				cp16.LinkMap[num58 - 100 + 120 + amountOfExtraAccessorySlotsToShow - 1].Left = 1570;
			};
			cp16.PageOnLeft = 8;
			cp16.PageOnRight = 8;
			...
			{
				int num56 = UILinkPointNavigator.Shortcuts.NPCS_IconsPerColumn;
				if (num56 == 0)
				{
					num56 = 100;
				}
				for (int num57 = 0; num57 < 50; num57++)
				{
					...
					if (cp14.LinkMap[600 + num57].Up == -1)
					{
						if (num57 >= num56 * 2)
						{
							cp14.LinkMap[600 + num57].Up = 307;
						}
						else if (num57 >= num56)
						{
							cp14.LinkMap[600 + num57].Up = 306;
						}
						else
						{
							cp14.LinkMap[600 + num57].Up = 305;
						}
					}
					cp14.LinkMap[600 + num57].Down = (((num57 + 1) % num56 == 0 || num57 == UILinkPointNavigator.Shortcuts.NPCS_IconsTotal - 1) ? 308 : (600 + num57 + 1));
					cp14.LinkMap[600 + num57].Left = ((num57 < UILinkPointNavigator.Shortcuts.NPCS_IconsTotal - num56) ? (600 + num57 + num56) : (-3));
					cp14.LinkMap[600 + num57].Right = ((num57 < num56) ? (-4) : (600 + num57 - num56));
				}
			};
			...
			cp12.UpdateEvent += delegate
			{
				...
				case 1:
				{
					...
					if (num55 == 100)
					{
						num55 = UILinkPointNavigator.Shortcuts.NPCS_IconsTotal;
					}
					cp12.LinkMap[308].Up = 600 + num55 - 1;
					break;
				}
				case 2:
			...
			{
				int num52 = UILinkPointNavigator.Shortcuts.CRAFT_CurrentIngridientsCount;
				int num53 = num52;
				...
				if (UILinkPointNavigator.OverridePoint == -1 && cp11.CurrentPoint > 1500 + num52)
				{
				...
				for (int num54 = 1; num54 < num52; num54++)
				{
					cp11.LinkMap[1500 + num54].Left = 1500 + num54 - 1;
					cp11.LinkMap[1500 + num54].Right = ((num54 == num52 - 2) ? (-4) : (1500 + num54 + 1));
				}
				cp11.LinkMap[1501].Left = -3;
				if (num52 > 0)
				{
					cp11.LinkMap[1500 + num52 - 1].Right = -4;
				}
				cp11.LinkMap[1500].Down = ((num52 >= 2) ? 1502 : (-2));
				cp11.LinkMap[1500].Left = ((num52 >= 1) ? 1501 : (-3));
			...
			for (int num18 = 700; num18 < 1500; num18++)
			{
				...
				uILinkPoint11.OnSpecialInteracts += delegate
				{
					...
					{
						int num51 = Main.recStart + IHateLambda - 700;
						if (Main.mouseItem.type == 0 && Main.recipe[Main.availableRecipe[num51]].createItem.maxStack > 1 && player.ItemSpace(Main.recipe[Main.availableRecipe[num51]].createItem).CanTakeItemToPersonalInventory && !player.HasLockedInventory())
						{
							flag2 = true;
							if (PlayerInput.Triggers.JustPressed.Grapple)
							{
								SomeVarsForUILinkers.SequencedCraftingCurrent = Main.recipe[Main.availableRecipe[num51]];
							}
							if (PlayerInput.Triggers.Current.Grapple && Main.stackSplit <= 1)
							{
								ItemSlot.RefreshStackSplitCooldown();
								if (SomeVarsForUILinkers.SequencedCraftingCurrent == Main.recipe[Main.availableRecipe[num51]])
								{
									Main.CraftItem(Main.recipe[Main.availableRecipe[num51]]);
			...
			{
				int num48 = UILinkPointNavigator.Shortcuts.CRAFT_IconsPerRow;
				int cRAFT_IconsPerColumn = UILinkPointNavigator.Shortcuts.CRAFT_IconsPerColumn;
				if (num48 == 0)
				{
					num48 = 100;
				}
				int num49 = num48 * cRAFT_IconsPerColumn;
				if (num49 > 800)
				{
					num49 = 800;
				}
				if (num49 > Main.numAvailableRecipes)
				{
					num49 = Main.numAvailableRecipes;
				}
				for (int num50 = 0; num50 < num49; num50++)
				{
					cp10.LinkMap[700 + num50].Left = ((num50 % num48 == 0) ? (-3) : (700 + num50 - 1));
					cp10.LinkMap[700 + num50].Right = (((num50 + 1) % num48 == 0 || num50 == Main.numAvailableRecipes - 1) ? (-4) : (700 + num50 + 1));
					cp10.LinkMap[700 + num50].Down = ((num50 < num49 - num48) ? (700 + num50 + num48) : (-2));
					cp10.LinkMap[700 + num50].Up = ((num50 < num48) ? (-1) : (700 + num50 - num48));
				}
			...
			cp9.UpdateEvent += delegate
			{
				...
				{
					cp9.LinkMap[2605 + num47].Left = ((num47 % num44 == 0) ? (-3) : (2605 + num47 - 1));
					cp9.LinkMap[2605 + num47].Right = (((num47 + 1) % num44 == 0 || num47 == count - 1) ? (-4) : (2605 + num47 + 1));
					cp9.LinkMap[2605 + num47].Down = ((num47 < num46 - num44) ? (2605 + num47 + num44) : (-2));
					cp9.LinkMap[2605 + num47].Up = ((num47 < num44) ? (-1) : (2605 + num47 - num44));
				}
			};
			...
			cp8.UpdateEvent += delegate
			{
				...
				if (num40 < 4 && PlayerInput.Triggers.JustPressed.MouseLeft)
				{
				...
				if (num40 == 4 + num41 && PlayerInput.Triggers.JustPressed.MouseLeft)
				{

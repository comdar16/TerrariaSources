...
namespace Terraria.GameContent.Tile_Entities
{
	public class TEDisplayDoll : TileEntity
	{
		...
		{
			return Position.X + "x  " + Position.Y + "y item: " + _items[0]?.ToString() + " " + _items[1]?.ToString() + " " + _items[2];
		}
		...
		public static void Framing_CheckTile(int callX, int callY)
		{
			...
			{
				Item.NewItem(num * 16, num2 * 16, 32, 48, 498);
			}
			else
			{
				Item.NewItem(num * 16, num2 * 16, 32, 48, 1989);
			}

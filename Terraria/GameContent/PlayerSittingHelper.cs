...
namespace Terraria.GameContent
{
	public struct PlayerSittingHelper
	{
		...
		public void UpdateSitting(Player player)
		{
			...
			if (!GetSittingTargetInfo(player, coords.X, coords.Y, out var targetDirection, out var _, out var seatDownOffset))
			{
				SitUp(player);
				return;
			}
		...
		{
			if (!GetSittingTargetInfo(player, x, y, out var targetDirection, out var playerSittingPosition, out var seatDownOffset))
			{
				return;
			}

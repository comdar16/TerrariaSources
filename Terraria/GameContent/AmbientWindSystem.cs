...
namespace Terraria.GameContent
{
	public class AmbientWindSystem
	{
		...
		private void SpawnAirborneCloud(int x, int y)
		{
			...
			float num2 = 1.1f;
			float num3 = 2.2f;
			float num4 = (float)Math.PI * 3f / 400f * _random.NextFloatDirection();
			float num5 = (float)Math.PI * 3f / 400f * _random.NextFloatDirection();
			while (num5 > (float)Math.PI * -3f / 800f && num5 < (float)Math.PI * 3f / 800f)
			{
				num5 = (float)Math.PI * 3f / 400f * _random.NextFloatDirection();
			}
			if (_random.Next(4) == 0)
			{

...
namespace Terraria.GameContent
{
	public struct VoidLensHelper
	{
		...
		public void DrawToDrawData(List<DrawData> drawDataList, int selectionMode)
		{
			...
			color *= _opacity;
			DrawData drawData = new DrawData(val.Value, _position - Main.screenPosition, rectangle, color, 0f, rectangle.Size() / 2f, 1f, SpriteEffects.None, 0);
			...
			if (selectionMode != 0)
			{
				...
				if (num2 > 10)
				{
					...
					drawData = new DrawData(TextureAssets.Extra[93].Value, _position - Main.screenPosition, rectangle, selectionGlowColor, 0f, rectangle.Size() / 2f, 1f, SpriteEffects.None, 0);
					drawDataList.Add(drawData);
				}
			}
		}

	...
	public class CreativePowerManager
	{
		...
		public void Reset()
		{
			foreach (KeyValuePair<ushort, ICreativePower> item in _powersById)
			{
				...
				(item.Value as IPersistentPerWorldContent)?.Reset();
				(item.Value as IPersistentPerPlayerContent)?.Reset();
			}
		}
		...
		public void LoadToPlayer(Player player, BinaryReader reader, int versionGameWasLastSavedOn)
		{
			while (reader.ReadBoolean())
			{
				...
				(value as IPersistentPerPlayerContent)?.Load(player, reader, versionGameWasLastSavedOn);
			}
		...
		public void ApplyLoadedDataToPlayer(Player player)
		{
			foreach (KeyValuePair<ushort, ICreativePower> item in _powersById)
			{
				(item.Value as IPersistentPerPlayerContent)?.ApplyLoadedDataToOutOfPlayerFields(player);
			}
		}
		...
		public void ResetPowersForPlayer(Player player)
		{
			foreach (KeyValuePair<ushort, ICreativePower> item in _powersById)
			{
				(item.Value as IPersistentPerPlayerContent)?.ResetDataForNewPlayer(player);
			}
		}

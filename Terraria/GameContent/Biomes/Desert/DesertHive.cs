...
namespace Terraria.GameContent.Biomes.Desert
{
	public static class DesertHive
	{
		...
		private static void PlaceClustersArea(DesertDescription description, ClusterGroup clusters, Rectangle area, PostPlacementEffect[,] postEffectMap, Point postEffectMapOffset)
		{
			...
			Vector2 vector = new Vector2(description.Hive.Width, description.Hive.Height);
			Vector2 vector2 = new Vector2(clusters.Width, clusters.Height);
			Vector2 vector3 = description.BlockScale / 2f;
			for (int i = area.Left; i < area.Right; i++)
			{
				for (int j = area.Top; j < area.Bottom; j++)
				{
					...
					Vector2 value = (new Vector2(num4, num5) - vector3) / vector * vector2;
					for (int k = 0; k < clusters.Count; k++)
					{
						...
						{
							num6 += 1f / Vector2.DistanceSquared(item.Position, value);
						}
						if (num6 > num)
						{
							if (num > num3)
					...
					bool flag = ((new Vector2(num4, num5) - vector3) / vector * 2f - Vector2.One).Length() >= 0.8f;

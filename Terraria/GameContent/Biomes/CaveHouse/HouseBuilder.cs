			...
			foreach (Rectangle room in Rooms)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					case 0:
					{
						...
						Vector2 vector = ((Type != HouseType.Desert) ? WorldGen.randHousePicture() : WorldGen.RandHousePictureDesert());
						int type = (int)vector.X;
						int style = (int)vector.Y;
						WorldGen.PlaceTile(num4, num5, type, mute: true, forced: false, -1, style);
						break;
					}
					case 1:
					{
				...
				while (num6 > 0)
				{
					...
					case 2:
					{
						...
						WorldGen.PlaceTile(num7, num8, WorldGen.statueList[num9].X, mute: true, forced: false, -1, WorldGen.statueList[num9].Y);

		...
		public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
		{
			...
			for (int j = num; j < num2; j++)
			{
				if (_slimes[j].Active)
				{
					...
					Vector2 vector3 = _slimes[j].Position;
					...
					{
						spriteBatch.Draw(_slimes[j].Texture, vector3, _slimes[j].GetSourceRectangle(), color, 0f, Vector2.Zero, vector2.X * 2f, SpriteEffects.None, 0f);
					}
				}
			}
		}

namespace Terraria.GameContent.Achievements
{
	public class AchievementsHelper
	{
		...
		internal static void OnPlayerEnteredWorld(Player player)
		{
			...
			for (int num4 = 0; num4 < player.armor.Length; num4++)
			{
				if (player.armor[num4].wingSlot > 0)
				{
			...
			{
				if (player.IsAValidEquipmentSlotForIteration(num5) && (player.dye[num5].type < 1 || player.dye[num5].stack < 1))
				{
					flag = false;
				}
			}
		...
		public static void HandleOnEquip(Player player, Item item, int context)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				if (player.IsAValidEquipmentSlotForIteration(i) && (player.dye[i].type < 1 || player.dye[i].stack < 1))
				{
					return;
				}
			}

	...
	public class Gore
	{
		...
		public void Update()
		{
			...
			if (type >= 907 && type <= 909)
			{
				...
				Vector4 vector3 = Collision.SlopeCollision(position, velocity, num36, num36, 0f, fall: true);
		...
		private void Gore_UpdateLeaf()
		{
			...
			if (velocity.Y < 0f)
			{
				...
				v = Collision.TileCollision(vector, v, num, num);
				if (flag)
				{
			...
			vector3 = Collision.TileCollision(vector, vector3, num3, num3);
			Vector4 vector5 = Collision.SlopeCollision(vector, vector3, num3, num3, 1f);

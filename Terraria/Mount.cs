...
namespace Terraria
{
	public class Mount
	{
		...
		public IProjectileSource GetProjectileSpawnSource(Player mountedPlayer)
		{
			return new ProjectileSource_Mount(mountedPlayer, Type);
		}
		...
		public void StartAbilityCharge(Player mountedPlayer)
		{
			if (Main.myPlayer == mountedPlayer.whoAmI)
			{
				if (_type == 9)
				{
					...
					Projectile.NewProjectile(GetProjectileSpawnSource(mountedPlayer), num, num2, 0f, 0f, type, 0, 0f, mountedPlayer.whoAmI, ai, ai2);
		...
		public void UpdateFrame(Player mountedPlayer, int state, Vector2 velocity)
		{
			...
			case 47:
			{
				...
				if (state == 1)
				{
					...
					if ((_type == 10 || _type == 47) && flag4)
					{
						...
						{
							Dust dust3 = Main.dust[Dust.NewDust(vector5, 0, 0, type)];
							Vector2 vector7 = Vector2.UnitY.RotatedBy(num11 * ((float)Math.PI * 2f) / 14f + num10);
							vector7 *= 0.2f * (float)_frameExtra;
							dust3.position = vector5 + vector7 * vector6;
							dust3.velocity = vector7 + new Vector2(RunSpeed - (float)(Math.Sign(velocity.X) * _frameExtra * 2), 0f);
			...
			case 44:
			{
				...
				else if (!flag2 && flag)
				{
					...
					for (float num6 = 0f; num6 < 20f; num6 += 1f)
					{
						...
						Vector2 vector4 = Vector2.UnitY.RotatedBy(num6 * ((float)Math.PI * 2f) / 20f + num5);
						vector4 *= 0.8f;
			...
			case 45:
			{
				...
				else if (!flag6 && flag5)
				{
					...
					{
						Dust dust4 = Main.dust[Dust.NewDust(vector8, 0, 0, 6)];
						Vector2 vector10 = Vector2.UnitY.RotatedBy(num15 * ((float)Math.PI * 2f) / 20f + num14);
						vector10 *= 0.8f;
						dust4.position = vector8 + vector10 * vector9;
						dust4.velocity = vector10 + new Vector2(RunSpeed - (float)Math.Sign(velocity.Length()), 0f);
						if (velocity.X > 0f)
						{
							dust4.velocity.X *= -1f;
						}
				...
				if (flag5)
				{
					int num16 = Utils.SelectRandom(Main.rand, new short[3] { 6, 6, 31 });
					int num17 = 6;
		...
		public void UpdateEffects(Player mountedPlayer)
		{
			...
			case 46:
			{
				...
				for (int j = 0; j < 200; j++)
				{
					...
					if (num13 < num12 && ((Vector2.Distance(vector10, center) > num13 && num13 < num11) || !flag))
					{
						bool flag2 = true;
			...
			case 11:
			{
				...
				if (Main.netMode != 2)
				{
					...
					Vector2 vector5 = new Vector2(num5 * (float)num4, -19f).RotatedBy(mountedPlayer.fullRotation);
					...
					{
						int num6 = (int)Vector2.Distance(vector7, vector8) / 3;
						...
						for (float num7 = 1f; num7 <= (float)num6; num7 += 1f)
						{
							...
							obj.position = Vector2.Lerp(vector8, vector7, num7 / (float)num6);
							obj.noGravity = true;
				...
				for (int i = 0; i < 200; i++)
				{
					...
					if (nPC.active && nPC.immune[mountedPlayer.whoAmI] <= 0 && !nPC.dontTakeDamage && nPC.Distance(minecartMechPoint) < 300f && nPC.CanBeChasedBy(mountedPlayer) && Collision.CanHitLine(nPC.position, nPC.width, nPC.height, minecartMechPoint, 0, 0) && Math.Abs(MathHelper.WrapAngle(MathHelper.WrapAngle(nPC.AngleFrom(minecartMechPoint)) - MathHelper.WrapAngle((mountedPlayer.fullRotation + (float)num4 == -1f) ? ((float)Math.PI) : 0f))) < (float)Math.PI / 4f)
					{
						...
						num9 += v.ToRotation();
						num8++;
						int num10 = Projectile.NewProjectile(GetProjectileSpawnSource(mountedPlayer), minecartMechPoint.X, minecartMechPoint.Y, v.X, v.Y, 591, 0, 0f, mountedPlayer.whoAmI, mountedPlayer.whoAmI);
		...
		public void Draw(List<DrawData> playerDrawData, int drawType, Player drawPlayer, Vector2 Position, Color drawColor, SpriteEffects playerEffect, float shadow)
		{
			...
			case 50:
				if (drawType == 0)
				{
					...
					Rectangle value6 = new Rectangle(0, num6 * _frameExtra, _data.textureWidth, num6);
					if (flag)
					{
						value6.Height -= 2;
					}
					item = new DrawData(TextureAssets.Extra[207].Value, position, value6, drawColor, num8, origin, scale, spriteEffects, 0);
			...
			case 35:
			{
				...
				Vector2 vector2 = new Vector2(num10, -26f);
				if (shadow == 0f)
				{
				...
				item = new DrawData(value4, Position + vector2, value5, drawColor, num8, origin, scale, spriteEffects, 0);
				item.shader = currentShader;
				playerDrawData.Add(item);
				break;
			}
			case 38:
				if (drawType == 0)
				{
					...
					Vector2 vector = new Vector2(num9, -10f);
					Texture2D value2 = TextureAssets.Extra[151].Value;
					Rectangle value3 = value2.Frame();
					item = new DrawData(value2, Position + vector, value3, drawColor, num8, origin, scale, spriteEffects, 0);
					item.shader = currentShader;
					playerDrawData.Add(item);
					flag2 = true;
				}
			...
			case 50:
				if (drawType == 0)
				{
					...
					item = new DrawData(texture2D, position3, value8, drawColor, num8, origin, scale, spriteEffects, 0);
					item.shader = currentShader;
					playerDrawData.Add(item);
				}
				break;
			...
			case 23:
				if (drawType == 0)
				{
					...
					item = new DrawData(texture2D, Position.Floor(), value, drawColor, num8, origin, scale, spriteEffects, 0);
					item.shader = currentShader;
					playerDrawData.Add(item);
					...
					for (int l = 0; l < 4; l++)
					{
						item = new DrawData(texture2D, (Position + ((float)l * ((float)Math.PI / 2f)).ToRotationVector2() * num12).Floor(), value, color4, num8, origin, scale, spriteEffects, 0);
						item.shader = currentShader;
						playerDrawData.Add(item);
					}
				}
			...
			case 8:
			{
				...
				for (int i = 0; i < drillMountData2.beams.Length; i++)
				{
					...
					for (int j = 0; j < 2; j++)
					{
						...
						float y = v.Length();
						item = new DrawData(scale: new Vector2(2f, y), texture: TextureAssets.MagicPixel.Value, position: vector6 + Position, sourceRect: value7, color: color2, rotation: num11 - (float)Math.PI / 2f, origin: Vector2.Zero, effect: SpriteEffects.None, inactiveLayerDepth: 0);
